...
3. **CTR ≥ 4% 的视频元数据冻结**——标题和封面已被算法验证为高效分发组合，绝对禁止改动任何元数据（标题/封面/标签），避免触发算法重新评估（Re-evaluation）导致降权。如需优化，只改描述区和评论区置顶。
4. **CTR < 2.5% 但 AVD 占比 ≥ 15% 的视频（门面拖累）**——这是"高质低进"问题：内容本身没问题，是门面（封面+标题）没把观众拉进来。策略动作只准开出"重置门面（Reset Facade）"——换封面+微调标题钩子，严禁发散到剧情节奏或选题方向。
...
10. **高CTR视频元数据冻结**：CTR ≥ 4% 的视频，其标题和封面已被算法验证为高效分发组合，绝对禁止改动任何元数据（标题/封面/标签），避免触发算法重新评估（Re-evaluation）导致降权。如需优化，只改描述区和评论区置顶。
...
- 播放分布：YouTube推荐系统遵循幂律分布（Power-law Distribution），头部视频播放占比高是平台机制的必然结果，严禁将其列为频道问题。诊断重点必须是"如何将头部视频的核心叙事骨架与封面×标题协同模式进行批量复制（Format Replication）"
...
- 基准: YouTube推荐系统遵循幂律分布，头部集中是平台机制的必然结果，严禁列为"频道问题"。诊断重点：头部视频的叙事骨架是什么？封面×标题协同模式是什么？如何批量复制（Format Replication）？
